...
*   **Фронтенд (Web):** Rust (Leptos + Tailwind CSS + Rust/UI Icons) + WASM.
...
*   **Контент:** Текст, реакции, пересылка сообщений, вложения (файлы, изображения). Использование компонентов Tailwind CSS для отображения аватаров, списков чатов и сообщений.
...
*   **Регистрация:** Стандартная регистрация для сотрудников (email/SSO). Использование готовых форм с Tailwind CSS.
...
*   **Для вебинаров/трансляций:** Количество подключений, время просмотра, действия пользователей. Отображение статистики через табличные компоненты Tailwind CSS (аналогично разделу "Payments" на сайте).
...
*   **UI Компоненты:** Использовать библиотеку **Tailwind CSS** в качестве основы для построения интерфейса. Применить готовые компоненты:
...
*   **Сборка:** Использование Tauri (бэкенд на Rust + фронтенд на Leptos с компонентами `Tailwind CSS`).
...
1.  Авторизация сотрудников и создание встречи "по ссылке".
...
4.  Обмен файлами в чате.
5.  Личные сообщения.
6.  Мультиязычность интерфейса (i18n).
6.  Шифрование TLS/DTLS.

### Этап 2: Основной функционал
1.  Корпоративный мессенджер (персистентные чаты) с UI на основе списков и аватаров.
...
1.  Брендирование страниц вебинаров.
2.  Интеграция с календарями (Google, Outlook) для бронирования переговорных комнат.
3.  Детальная аналитика для администраторов.

### Этап 5: Мобильные приложения (Android, iOS)
*   **Ядро:** Бизнес-логика и сетевое взаимодействие на Rust, скомпилированное в мобильную библиотеку (.so / .a).
*   **UI:** Нативные компоненты платформ или решение на базе Leptos с рендерингом через WebView (гибрид) или Tauri.

### Этап 6: Десктопные приложения (Windows, macOS, Linux)
*   **Сборка:** Использование Tauri (бэкенд на Rust + фронтенд на Leptos с компонентами).
*   **Функции:** Захват экрана (с высоким FPS), системные уведомления, автозапуск.

## 9. Требования к интеграциям
*   **API:** Возможность интеграции со сторонними системами (CRM/LMS/CMS) через API (api key) и IFrame.
...
*   Интерфейс, построенный на компонентах, корректо отображается в последних версиях Chrome, Firefox, Safari.
*   Интерфейс полностью адаптивен и удобен на мобильных устройствах и планшетах.

---

Документ содержит все ключевые детали: от высокоуровневой архитектуры до конкретных UI-решений.
